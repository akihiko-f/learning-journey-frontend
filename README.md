# React学習プロジェクト

プログラミング入門レベルからReactでwebアプリを作れるようになるための学習プロジェクトです。

## 📁 プロジェクト構成

```
learning-journey/
├── frontend/              # フロントエンドアプリ
│   ├── todo-app/          # ToDoアプリ（Phase 1完了）
│   ├── timer-app/         # カウントダウンタイマー（Phase 2-1完了）
│   ├── calculator-app/    # 簡易計算機（Phase 2-2完了）
│   ├── favorites-app/     # お気に入りリスト（Phase 2-3完了）
│   ├── memo-app/          # メモアプリ（Phase 3-1完了）
│   ├── budget-app/        # 家計簿アプリ（Phase 3-2完了）
│   ├── weather-app/       # 天気予報アプリ（Phase 3-3完了）
│   ├── portfolio-app/     # ポートフォリオサイト（Phase 4-1完了）
│   └── free-practice-app/ # 自由練習用アプリ
├── backend/               # バックエンド（今後の拡張用）
├── assignments/           # お題の仕様書
├── learning-journey/      # 学習記録
└── README.md
```

各アプリは完全に独立したプロジェクトです。

**free-practice-app について:**
- 学習スケジュールとは別に、自由に実験・練習するためのアプリ
- 写経なしで、自分でお題を考え、イメージしたものを少しずつコードにしていく
- React Routerを使ったランディングページなど、自由な実践の場

### 起動方法

```bash
# 各アプリを起動
cd frontend/<app-name>
npm install  # 初回のみ
npm run dev

# 例: 家計簿アプリを起動
cd frontend/budget-app
npm install  # 初回のみ
npm run dev
```

## 📚 学習記録

日々の学習記録は [`learning-journey/`](./learning-journey) ディレクトリに保存されています。

2025年10月16日から継続的に学習を記録しています。詳細は各日付のファイルを参照してください。

## 🚀 実装した機能

### ToDoアプリ（Phase 1完了）
- ✅ ToDoの追加
- ✅ ToDoの削除
- ✅ ToDoの編集（インライン編集）
- ✅ 完了/未完了チェックボックス
- ✅ 完了時の取り消し線スタイル
- ✅ 入力バリデーション（空のToDoは追加不可）
- ✅ LocalStorageでデータ永続化（ページリロード後もデータが残る）
- ✅ フィルター機能（全て/未完了/完了の切り替え）
- ✅ タブUI（アクティブ状態の視覚的表示）
- ✅ Unit Test（5個のテストケース）
- ✅ Storybook（4つのストーリー）
- ✅ CSS/スタイリング（Flexbox、ホバーエフェクト）

### カウントダウンタイマー（Phase 2-1完了）
- ✅ 時間設定（分・秒の入力欄）
- ✅ スタート/ストップ/リセットボタン
- ✅ MM:SS形式での残り時間表示
- ✅ 1秒ごとのカウントダウン
- ✅ 時間切れメッセージ表示
- ✅ タイマー実行中の入力欄無効化

### 簡易計算機（Phase 2-2完了）
- ✅ 四則演算（+、-、*、/）
- ✅ 連続計算対応（5 + 3 + 2など）
- ✅ クリアボタン
- ✅ 計算履歴の表示
- ✅ ゼロ除算対策
- ✅ CSS Grid レイアウト

### お気に入りリスト（Phase 2-3完了）
- ✅ アイテムの追加・削除
- ✅ お気に入りマーク（星アイコン★/☆）
- ✅ LocalStorageでデータ永続化
- ✅ レスポンシブなUIデザイン
- ✅ Flexboxレイアウト
- ✅ ホバーエフェクトとアニメーション

### メモアプリ（Phase 3-1完了）
- ✅ メモの作成（タイトル・本文）
- ✅ メモの編集（編集モードと新規作成モードの切り替え）
- ✅ メモの削除
- ✅ 作成日時の表示（ISO形式から読みやすい形式に変換）
- ✅ LocalStorageでデータ永続化
- ✅ 検索機能（タイトル・本文から検索）
- ✅ CSSスタイリング

### 家計簿アプリ（Phase 3-2完了）
- ✅ 収入・支出の記録
- ✅ カテゴリ分類（支出：食費、交通費、娯楽、光熱費、通信費、その他）
- ✅ カテゴリ分類（収入：給与、副業、お小遣い、その他）
- ✅ 合計金額の計算（収入・支出・残高）
- ✅ 月ごとの表示切り替え（フィルター機能）
- ✅ LocalStorageでデータ永続化
- ✅ エラーハンドリング（LocalStorage読み込み失敗時）
- ✅ **カテゴリ別支出グラフ（横棒グラフ）**
- ✅ **収入・支出比較グラフ（縦棒グラフ）**
- ✅ CSSスタイリング（グラデーション、アニメーション）

### 天気予報アプリ（Phase 3-3完了）
- ✅ OpenWeatherMap APIとの連携
- ✅ 都市名検索機能
- ✅ 天気情報の表示（気温、体感温度、湿度、風速）
- ✅ 天気アイコンの動的表示
- ✅ ローディングスピナー
- ✅ エラーハンドリング（都市が見つからない場合など）
- ✅ async/awaitを使った非同期処理
- ✅ レスポンシブデザイン

### ポートフォリオサイト（Phase 4-1完了）
- ✅ React Router（react-router-dom）の導入
- ✅ 複数ページの実装（Home、About、Projects、Contact、NotFound）
- ✅ ナビゲーションバーコンポーネント
- ✅ Linkコンポーネントによるページ遷移
- ✅ 動的ルート（/projects/:id）
- ✅ useParamsフックでURLパラメータ取得
- ✅ プロジェクト詳細ページ
- ✅ 404ページの実装
- ✅ グラデーション背景とアニメーション

## 🛠️ 技術スタック

- **フレームワーク**: React 19
- **ビルドツール**: Vite
- **テストフレームワーク**: Vitest
- **テストライブラリ**: React Testing Library
- **コンポーネントカタログ**: Storybook
- **言語**: JavaScript

## 📦 ToDoアプリ - セットアップとテスト

ToDoアプリのディレクトリで実行：

```bash
cd todo-app

# 依存パッケージのインストール
npm install

# 開発サーバーの起動
npm run dev

# テストの実行
npm run test

# Storybookの起動
npm run storybook
```

## 🧪 ToDoアプリ - テスト

ToDoアプリのディレクトリで実行：

### Unit Test実行

```bash
cd todo-app
npm run test
```

**テストケース:**
- タイトルが表示される
- ToDoを追加できる
- 空のToDoは追加できない
- ToDoを削除できる
- 複数のToDoを追加できる

### Storybook

```bash
cd todo-app
npm run storybook
```

ブラウザで `http://localhost:6006` を開くと、以下のストーリーが確認できます：

- **Empty**: 初期状態（空のリスト）
- **WithOneItem**: ToDoが1つある状態
- **WithMultipleItems**: 複数のToDoがある状態
- **WithLongText**: 長いテキストのToDo


## 🎯 学習計画とロードマップ

### 学習の進め方

#### 基本的な学習スタイル：写経（サンプルコードを書き写す）
1. サンプルコードを提示してもらう
2. 自分でコードを書き写す
3. 動作確認
4. **コードの全ての部分を理解するまで質問を繰り返す**
5. 学習記録を`learning-journey/`に追記
6. Gitにコミット・push

**すべてのフェーズで写経スタイルを採用** - お題に対してサンプルコードを提示してもらい、理解しながら実装していく

#### free-practice-appでの自由実践
- 学習スケジュールとは完全に独立
- 自分でお題を考え、自分で実装方針を決める
- 必要に応じてサポートを受けながら、自由に実験・チャレンジする場

---

### Phase 1: ToDoアプリの機能拡張（写経フェーズ）

#### ✅ 1. 完了/未完了チェックボックス
**学習内容：**
- オブジェクト配列でのstate管理
- 条件付きスタイル（完了時の取り消し線）
- チェックボックスのイベント処理

**実装機能：**
- ToDoオブジェクト（id, text, completed）
- チェックボックスUI
- 完了時の取り消し線

---

#### ✅ 2. ToDoの編集機能
**学習内容：**
- 条件付きレンダリング（編集モード切り替え）
- フォームのstate管理
- インラインで編集

**実装機能：**
- 編集ボタン
- 編集モードの切り替え
- 保存・キャンセル

---

#### ✅ 3. LocalStorageでデータ永続化
**学習内容：**
- useEffectフックの基礎
- LocalStorageのAPI
- JSONのシリアライズ・デシリアライズ

**実装機能：**
- ページリロード後もデータが残る
- 自動保存

---

#### ✅ 4. フィルター機能（全て/未完了/完了）
**学習内容：**
- 配列のfilterメソッド応用
- タブUIの実装
- 複数stateの管理

**実装機能：**
- 全て/未完了/完了の切り替え
- タブUI
- アクティブ状態の表示

---

#### ✅ 5. コンポーネント分割
**学習内容：**
- Propsの渡し方
- コンポーネント設計
- 再利用可能なコンポーネント

**実装機能：**
- TodoItemコンポーネント
- TodoListコンポーネント
- TodoFormコンポーネント

---

### Phase 2: 新しいアプリ開発（写経フェーズ）

各お題に対して**サンプルコードを提示**してもらい、写経しながら学習する。

#### ✅ お題1: カウントダウンタイマー
- 時間設定（分・秒）
- スタート/ストップ/リセット
- 時間切れの通知

**学習内容：** useStateとsetIntervalの理解

**完了日：** 2025/10/25

---

#### ✅ お題2: 簡易計算機
- 四則演算
- クリアボタン
- 計算履歴

**学習内容：** イベント処理と状態管理の理解

**完了日：** 2025/11/04

---

#### ✅ お題3: お気に入りリスト
- アイテムの追加・削除
- お気に入りマーク（星アイコン）
- LocalStorageで保存

**学習内容：** これまでの知識の統合

**完了日：** 2025/11/07

---

### Phase 3: 実践アプリ開発（写経フェーズ）

より複雑な機能を持つアプリケーションを、サンプルコードを参考に実装する。

#### ✅ お題4: メモアプリ
- メモの作成・編集・削除
- タイトルと本文
- 作成日時の表示
- LocalStorageで保存
- 検索機能

**学習内容：** より複雑なCRUD操作

**完了日：** 2025/11/08

---

#### ✅ お題5: 簡易家計簿アプリ
- 収入・支出の記録
- カテゴリ分類
- 合計金額の計算
- 月ごとの表示切り替え
- グラフ表示（カテゴリ別支出・収入支出比較）

**学習内容：** データの集計と表示、CSSグラフ実装

**完了日：** 2025/11/14

---

#### ✅ お題6: 天気予報アプリ
- 天気APIの利用
- 都市の検索
- 天気情報の表示
- ローディング状態の管理
- エラーハンドリング

**学習内容：** 外部APIとの連携

**完了日：** 2025/11/16

---

### Phase 4: 発展的なトピック（写経フェーズ）

Reactの高度な機能を、サンプルコードを参考に学習する。

#### ✅ 7. React Router（ページ遷移）
- 複数ページの管理
- URLパラメータ
- 動的ルート
- ナビゲーション

**学習内容：** SPAでのルーティング

**完了日：** 2025/11/26

---

#### ✅ 8. Context API（グローバル状態管理）
- コンポーネント間でのデータ共有
- Props Drillingの解決
- ダークモード/ライトモード切り替え
- LocalStorageとの連携

**学習内容：** グローバルな状態管理

**完了日：** 2025/11/29

---

#### ✅ 9. カスタムフック
- ロジックの再利用
- フックのルール
- よく使うパターン
- useLocalStorage、useToggle

**学習内容：** 再利用可能なロジックの作成

**完了日：** 2025/11/30

---

#### ✅ 10. パフォーマンス最適化
- useMemo, useCallback
- React.memo
- 3つの組み合わせパターン
- Strict Modeの理解

**学習内容：** アプリケーションの最適化

**完了日：** 2025/12/01

---

**Phase 4 完了！🎉**

次はPhase 5（実践的な開発プロセス）へ進みます。

### Phase 5: 実践的な開発プロセス（計画中）

要件定義からスタートし、実務に近い形でアプリケーション開発を学習する。

#### 開発プロセス
1. **要件定義**
   - 何を作るか明確化
   - ユーザーストーリー作成
   - 機能要件・非機能要件の洗い出し

2. **設計・ドキュメント作成**
   - `docs/`ディレクトリに設計ドキュメントを作成
   - アーキテクチャ設計
   - データベース設計
   - API設計

3. **実装**
   - Next.jsによるフルスタック開発
   - フロントエンド（React）
   - バックエンド（API Routes / Route Handlers）
   - データベース連携

**学習内容：** 実務に即した開発フロー、要件定義力、設計力、ドキュメンテーション能力

---

### Phase 6: Next.jsによるフルスタック開発（計画中）

#### 学習項目
1. **Next.js基礎**
   - App Router
   - Server Components vs Client Components
   - ファイルベースルーティング

2. **バックエンド開発**
   - API Routes / Route Handlers
   - データベース連携（Prisma/Drizzle）
   - 認証・認可（NextAuth.js）

3. **フロントエンド開発**
   - React Server Components
   - フォーム処理（Server Actions）
   - データフェッチング戦略

4. **デプロイ**
   - Vercelへのデプロイ
   - 環境変数管理
   - 本番環境の設定

**学習内容：** モダンなフルスタック開発、Next.jsのベストプラクティス

---

### 学習時間目安

- Phase 1（基本機能拡張）：各項目 1-2時間
- Phase 2（新アプリ開発）：各項目 2-4時間
- Phase 3（実践アプリ）：各項目 4-8時間
- Phase 4（発展トピック）：各項目 2-3時間
- Phase 5（実践開発プロセス）：8-12時間
- Phase 6（Next.jsフルスタック）：20-30時間

**Phase 1-4合計：** 約50-80時間
**Phase 5-6合計：** 約28-42時間
**総合計：** 約78-122時間

**学習スタイル:**
- Phase 1-4: 写経スタイル（サンプルコードを提示、理解しながら実装）
- Phase 5-6: 実践スタイル（要件定義から設計、実装まで実務に即した流れ）

---

### チェックポイント

各項目完了時に確認：
- [ ] コードが動作する
- [ ] コードの全ての部分を説明できる
- [ ] 同じ機能を別の方法でも実装できる（理解度確認）
- [ ] テストを書ける
- [ ] Gitにコミット済み
- [ ] `learning-journey/`に学習記録追加済み

## 📝 学習リソース

- [React公式ドキュメント](https://react.dev/)
- [Vite公式ドキュメント](https://vite.dev/)
- [React Testing Libraryドキュメント](https://testing-library.com/docs/react-testing-library/intro/)
- [Storybookドキュメント](https://storybook.js.org/docs/)

---

🤖 このプロジェクトは[Claude Code](https://claude.com/claude-code)と一緒に学習しながら作成しました。
